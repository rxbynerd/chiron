...
// acceptance criteria and §8 golden-report requirement. The templated variant
// loads a report template fixture and also pins the system prompt the model
// received.
func TestWorkerGoldenReportThroughRunCore(t *testing.T) {
	for _, tt := range []struct {
		name string
		// template is a testdata fixture; empty selects the built-in format.
		template     string
		answer       string
		reportGolden string
		// promptGolden, when set, pins the first request's system prompt.
		promptGolden string
	}{
		{
			...
			reportGolden: "worker-report.md",
		},
		{
			name:     "report template",
			template: "worker-report-template.md",
			answer: "## Summary\n\n" +
				"10BASE-T1L is a long-reach single-pair Ethernet physical layer " +
				"standardised in IEEE 802.3cg.\n\n" +
				"## Key facts\n\n" +
				"- It runs over a single twisted pair.\n" +
				"- It targets industrial and building automation.\n",
			reportGolden: "worker-report-templated.md",
			promptGolden: "worker-system-prompt-templated.txt",
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			searchSrv := searchtest.NewFakeServer([]search.Result{
				...
				// A degraded result with no URL — the worker must tolerate it
				// (it is surfaced but not fetchable).
			...
			modelSrv := modeltest.NewFakeServer(
				...
				modeltest.FakeReply{
					Content:      `{"action":"final","answer":` + jsonString(tt.answer) + `,"citations":[{"url":"` + page.URL + `","title":"IEEE 802.3cg overview"}]}`,
					FinishReason: "stop",
			...
			if tt.template != "" {
				rt, err := LoadReportTemplate(filepath.Join("testdata", tt.template))
				if err != nil {
					t.Fatalf("LoadReportTemplate: %v", err)
				}
				workerDeps.ReportTemplate = rt
			}
			...
			checkGolden(t, tt.reportGolden, normalise(cap.result.Report.Markdown))
			if tt.promptGolden != "" {
				reqs := modelSrv.Requests()
				if len(reqs) == 0 {
					t.Fatal("the model received no requests")
				}
				checkGolden(t, tt.promptGolden, []byte(reqs[0].Messages[0].Content))
			}

			// Sanity beyond the golden: the search-derived source is cited,
			// and the URL-less degraded result never became a citation.
...
// checkGolden compares got with testdata/name, rewriting the file first when
// -update is set.
