...
| `internal/researcher/fleet` | v2 in-process research agents. `RunWorker` is the bounded search→fetch→final loop on one standard model (turn, token, GBP-ceiling and wall-clock caps; tool failures fed back to the model with a three-strike bound; citations restricted to URLs it saw and, with a knowledge store, locators it recalled), plus a `recall` action when `WorkerDeps.Knowledge` is set; `Worker` wraps it as a `Researcher` with opaque `wkr_` ids that `get`/`follow-up` refuse, and saves a completed finding back when `WorkerDeps.Remember` is set (docs/KNOWLEDGE.md). The remaining files implement `--agent fleet`, a lead over several workers. `fleet.go`: `Fleet`, the `Researcher` with opaque `flt_` ids that `get`/`follow-up` refuse; one `ContextStore` session per run; the lead runs detached from the caller's context, and `Await` cancels and waits out a bounded unwind. `lead.go`: one strict structured-output decompose call, never retried, into 3–5 briefs, persisted as the run's plan. `router.go`: the brief-target table, external web only, shared by the lead's validation and the pool. `pool.go`: runs at most `MaxWorkers` briefs, `Concurrency` at once, each a `RunWorker` under the shared `Caps`, and stores each finding by reference as it finishes. `synthesise.go`: reads the findings back by reference and writes the report in one plain-text call, stitching the findings on failure. `cite.go`: one structured call attributing the report's claims to worker-cited URLs. `conclude.go`: status rules and the run's single `Usage` rollup, with the lead priced from the workers' `Caps`. Fleet workers never save back and never receive the report template. |
| `internal/researcher/fleet/search` | The web-search tool's result-shape layer over `internal/mcpclient`: calls `search` and maps `{results:[{title,url,snippet}]}`, degrading to a prose snippet rather than erroring. `Options.Endpoint`/key ref are security-sensitive for the same reason as the model client. |
...
| `internal/mcpclient` | Hand-rolled Streamable-HTTP MCP transport shared by search and Billet: `initialize` handshake, `tools/call` with bounded JSON or SSE responses, best-effort session DELETE, cross-host and https-to-http redirects refused, key header-only and scrubbed, no retries. |
...
| `internal/trace` | `Tracer` seam with three bindings: OTel (OTLP/HTTP), JSONL (local debug), Noop. `names.go` fixes the span/metric vocabulary, including the reserved fleet span names (`decompose`, `delegate`, `synthesise`, `cite`, `control_plane`) and attribute keys (`worker_id`, `brief_id` on a delegate span; `brief_count` on decompose), pinned by a vocabulary test; all payloads scrubbed. |
...
| `internal/memory` | The agentic-memory seam, declared locally (never imported from any store's module; see DECISIONS.md). `Recaller`/`Rememberer` are long-term memory, bound to external knowledge stores by the subpackages below and consumed by the worker's recall action and save-back. `ContextStore` embeds both plus the session and artifact plane, bound to `Noop` or `InMemory`. `InMemory` is the in-process store the fleet passes its plan and findings through by reference (`fleet.memory: inmemory`, required for `--agent fleet`; the composition root binds one per fleet run and none for any other agent): SHA-256 content-addressed, immutable artifacts in session-scoped namespaces with an absolute TTL, a per-artifact size bound, and copies in and out. Its `Remember`/`Recall` return `ErrNotImplemented`. |
| `internal/memory/billet` | Billet adapter over `internal/mcpclient`: `Recall` calls `search_memory` (no degrade-to-prose: a reply without `records` is an error; each record bounded on read), `Remember` calls `save_memory` (content over 256 KiB refused before any request). Locators are `billet://memory/<id>`. Ships the `CHIRON_BILLET_BIN` interop test. |
...
- `fleet.knowledge_endpoint` / `fleet.knowledge_key_ref` — the knowledge
  ...
  must be empty or default. `fleet.knowledge_remember` (Billet and
  `--agent worker` only; rejected for `fleet`) sends
...
- The Gemini-only levers (`budget`, `plan`, `accept_plan`, `model`,
  ...
  for `worker`/`fleet` it gates whether per-turn `worker_turn` progress
  events reach the transport (`--quiet` disables them), but it carries no
  spend meaning there. `template` is honoured: it replaces the worker's
  output-format block, or for `fleet` the lead's synthesis format
  (`fleet.LoadReportTemplate`, bounded at 8 KiB).
