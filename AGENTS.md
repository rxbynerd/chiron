...
| `internal/researcher/fleet/search` | The web-search tool's result-shape layer over `internal/mcpclient`: calls `search` and maps `{results:[{title,url,snippet}]}`, degrading to a prose snippet rather than erroring; `Close` ends its MCP session. `Options.Endpoint`/key ref are security-sensitive for the same reason as the model client. |
